...
function Applications() {
    ...
    const onSearch = (e) => {
        const filterd = apiData.filter(x => `${x.arn}${x.fullName}${x.mobile}${x.email}`.toLowerCase().includes(e.target.value.toLowerCase()));
        setData([...filterd]);
    }
    ...
    return (
        <>
            ...
            <StatusUpdateModal open={current !== null} onClose={onModalclose} data={current} />
                ...
                {!loading && <>
                    <Stack direction="horizontal" gap={3}>
                        ...
                        <div className="p-2 ms-auto">
                            <Form.Label className="visually-hidden">Filter text (Name/Email/Mobile/ARN)</Form.Label>
                        ...
                        <Card.Body>
                            <Row>
                                <Col sm={12} lg={4}>
                                    ...
                                    <div>Pan: <small>{item.pan}</small></div>
                                    <div>Aadhaar: <small>{item.aadhaar}</small></div>
                                    <div> <svg aria-hidden="true" style={{ marginRight: 8 }} width="16px" height="16px" viewBox="0 0 28 28">
                            ...
                            <div style={{ textAlign: 'right' }}>
                                <Button variant="outline-primary" aria-label="View documents"
                                 onClick={() => downloadFiles(item.documentsBase64)} size="sm" style={{ marginRight: 16 }}>
                                ...
                                <Button variant="outline-primary" aria-label="Update application status"
                                onClick={() => onUpdate(item)} size="sm">
